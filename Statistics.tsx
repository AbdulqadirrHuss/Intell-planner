...
const getWeekNumber = (dateParam: Date) => {
    const dateObj = new Date(Date.UTC(dateParam.getFullYear(), dateParam.getMonth(), dateParam.getDate()));
    ...
    return [dateObj.getUTCFullYear(), weekNo];
};
...
const Statistics: React.FC<StatisticsProps> = ({
    ...
    const generateTableColumns = () => {
        ...
        if (tableView === 'daily') {
            ...
            for (let i = 0; i < 5; i++) {
                ...
                iterDate.setDate(start.getDate() + (i * 7));
                if (iterDate >= MIN_DATE) cols.push(formatDate(iterDate));
            }
    ...
    const navigateTable = (direction: number) => {
        const refDateObj = new Date(tableRefDate);
        if (tableView === 'daily') refDateObj.setDate(refDateObj.getDate() + direction);
        else if (tableView === 'weekly') refDateObj.setDate(refDateObj.getDate() + (direction * 35));
        else if (tableView === 'monthly') refDateObj.setFullYear(refDateObj.getFullYear() + direction);
        setTableRefDate(formatDate(refDateObj));
    };
    ...
    const getAggregatedValue = (dates: string[], def?: StatDefinition) => {
        ...
        dates.forEach(dateStr => {
            if (!def) { // Global
                const val = getGlobalCompletion(dateStr);
                if (val >= 0) { sum += val; count++; }
            } else {
                const val = getCellValue(dateStr, def);
                if (val !== null) {
    ...
    const groupGraphDates = () => {
        ...
        graphDates.forEach(date => {
            const dateObj = new Date(date);
            let key = '';
            if (graphView === 'weekly') key = `Week ${getWeekNumber(dateObj)[1]}, ${getWeekNumber(dateObj)[0]}`;
            else if (graphView === 'monthly') key = dateObj.toLocaleString('default', { month: 'short', year: 'numeric' });
            else if (graphView === 'yearly') key = dateObj.getFullYear().toString();
        ...
        <div className="flex bg-gray-800 rounded-lg p-1 gap-1 inline-flex">
            {VIEW_OPTIONS.map(opt => (
                <button
                    ...
                    className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${currentMode === opt.toLowerCase()
...
const ImprovedLineChart = ({ data, color = '#818cf8' }: any) => {
    ...
    const points = data.map((point: any, i: number) => `${getX(i)},${getY(point.value)}`).join(' ');
    return (
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-full overflow-visible">
            <polyline points={points} fill="none" stroke={color} strokeWidth="3" />
            {data.map((point: any, i: number) => (i % Math.ceil(data.length / 10) === 0) && (
                <text key={i} x={getX(i)} y={HEIGHT - 10} textAnchor="middle" fill="#94a3b8" fontSize="12">{point.label}</text>
            ))}
        </svg>
    );
};
...
const ImprovedBarChart = ({ data }: any) => {
    if (!data || data.length === 0) return null;
    const WIDTH = 1000, HEIGHT = 300, PAD = 40;
    const maxVal = Math.max(...data.map((point: any) => point.value), 10) * 1.1;
    const barW = (WIDTH - PAD * 2) / data.length * 0.6;
    return (
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-full overflow-visible">
            {data.map((point: any, i: number) => {
                const h = (point.value / maxVal) * (HEIGHT - PAD * 2);
                const x = PAD + i * ((WIDTH - PAD * 2) / data.length) + 20;
                const y = HEIGHT - PAD - h;
                // Change color if goal_direction is 'down' and value is high (bad)
                let barColor = point.color || '#6366f1';
                if (point.goal_direction === 'down' && point.value > (point.target || 50)) {
                    barColor = '#ef4444'; // Red for bad performance
                }
                return (
                    <g key={i}>
                        <rect x={x} y={y} width={barW} height={h} fill={barColor} rx="4" />
                        <text x={x + barW / 2} y={HEIGHT - 10} textAnchor="middle" fill="#94a3b8" fontSize="12">{point.name}</text>
                        <text x={x + barW / 2} y={y - 5} textAnchor="middle" fill="white" fontSize="12">{point.value}</text>
                    </g>
                );
            })}
        </svg>
