...
// Only create the client if keys exist. This prevents the "White Screen of Death".
    ...
    return (
      <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center justify-center p-8 text-center">
        <div className="bg-red-500/10 p-6 rounded-xl border border-red-500/50 max-w-lg">
          ...
          <div className="bg-black/50 p-4 rounded text-left font-mono text-xs text-gray-400">
            <p>Please ensure your <strong>.env.local</strong> file exists and contains:</p>
            <p className="mt-2 text-yellow-300">VITE_SUPABASE_URL=...</p>
            <p className="text-yellow-300">VITE_SUPABASE_ANON_KEY=...</p>
          </div>
          <button onClick={() => window.location.reload()} className="mt-6 bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded-lg w-full transition-colors">
            Reload App
          </button>
        </div>
      </div>
  ...
  useEffect(() => {
    async function loadInitialData() {
      ...
      try {
        ...
        // Fetch Uncategorized Routines (Global Habits)
        ...
        if (dtData && linksData) {
            const formattedDayTypes = dtData.map(dt => {
                ...
                // Attach Global Routines to ALL day types for now
