...
const MetricsGraph: React.FC<MetricsGraphProps> = ({
    ...
    if (type === 'bar') {
        ...
        return (
            <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-full overflow-visible">
                ...
                {data.map((d, i) => {
                    const h = (d.value / maxY) * (HEIGHT - PAD * 2);
                    ...
                    return (
                        <g key={i} className="group">
                            ...
                            <text x={x + barW / 2} y={y - 5} textAnchor="middle" fill="white" fontSize="12" className="opacity-0 group-hover:opacity-100 transition-opacity">{d.value}</text>
                            <text x={x + barW / 2} y={HEIGHT - 15} textAnchor="middle" fill="#94a3b8" fontSize="10">{d.label}</text>
                        </g>
                    );
                })}
            </svg>
    ...
    return (
        <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-full overflow-visible">
            ...
            {data.map((d, i) => (
                <g key={i} className="group">
                    <circle cx={getX(i)} cy={getY(d.value)} r="4" fill={color} stroke="#1e293b" strokeWidth="2" className="group-hover:r-6 transition-all" />

                    {/* Tooltip */}
                    <g className="opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                        <rect x={getX(i) - 20} y={getY(d.value) - 35} width="40" height="25" rx="4" fill="#1e293b" />
                        <text x={getX(i)} y={getY(d.value) - 18} textAnchor="middle" fill="white" fontSize="12" fontWeight="bold">{d.value}</text>
                    </g>

                    {/* X Axis Label (Show every nth label to avoid crowding) */}
                    {(i % Math.ceil(data.length / 10) === 0 || i === data.length - 1) && (
                        <text x={getX(i)} y={HEIGHT - 15} textAnchor="middle" fill="#94a3b8" fontSize="10">{d.label}</text>
                    )}
                </g>
            ))}
        </svg>
